...
class InMemoryCandleRepository() : CandleRepository {
    ...
    override fun size(symbol: SymbolId, timeframe: Timeframe): Int = store[symbol to timeframe]?.size ?: 0
    ...
    private fun mergeAndLimit(
        ...
        return if (ordered.size <= maxWindow) ordered
        else ordered.takeLast(maxWindow)
    }
